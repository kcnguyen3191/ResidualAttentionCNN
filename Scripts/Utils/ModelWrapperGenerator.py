        ...
        for epoch in range(initial_epoch, initial_epoch + epochs):
            ...
            for x_true, y_true in train_loader:
                ...
                def closure():
                    ...
                    # if loss is list, use PCGrad
                    if isinstance(self.loss, list): 
                        self.train_loss = []
                        for loss_fun in self.loss:
                            self.train_loss.append(loss_fun(y_pred, y_true))
                        self.optimizer.pc_backward(self.train_loss)
                ...
                if x_true.device != torch.device('cpu'):
